...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 05-09-2025 a las 03:07:59
...
INSERT INTO `finca` (`id_finca`, `nombre_finca`, `localizacion`, `correo`, `telefono`, `nombreEncargado`, `pais`, `departamento`, `ciudad`) VALUES
(1, 'El Mirador', 'Cachipay, Cundinamarca', 'qq@gmail.com', '12312313', 'Agapito', 'Colombia', 'Cundinamarca', 'cachipay');
...
--
-- Estructura de tabla para la tabla `manejo_pasto`
--

CREATE TABLE `manejo_pasto` (
  `id_manejo` int(11) NOT NULL,
  `id_potrero` smallint(6) NOT NULL,
  `fecha_medicion` date NOT NULL,
  `altura_pasto` decimal(5,2) DEFAULT NULL COMMENT 'Altura en centímetros',
  `cobertura` decimal(5,2) DEFAULT NULL COMMENT 'Porcentaje de cobertura',
  `estado_pasto` enum('excelente','bueno','regular','malo') NOT NULL,
  `observaciones` text DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
ALTER TABLE `cria`
  ...
  ADD KEY `id_animal` (`id_animal`),
  ADD KEY `cria_ibfk_1_new` (`id_padre`),
  ADD KEY `cria_ibfk_2_new` (`id_madre`);
...
--
-- Indices de la tabla `manejo_pasto`
--
ALTER TABLE `manejo_pasto`
  ADD PRIMARY KEY (`id_manejo`),
  ADD KEY `fk_manejo_potrero` (`id_potrero`);
...
ALTER TABLE `rotacion_potrero`
  ...
  ADD KEY `fk_rotacion_grupo` (`id_grupo_animal`);
...
--
-- Modificar la restricción de clave foránea para usuario_finca con finca
ALTER TABLE `usuario_finca` DROP FOREIGN KEY `fk_usuario_finca_finca`;
ALTER TABLE `usuario_finca` ADD CONSTRAINT `fk_usuario_finca_finca` 
  FOREIGN KEY (`finca_id`) REFERENCES `finca` (`id_finca`) ON DELETE CASCADE;
COMMIT;
